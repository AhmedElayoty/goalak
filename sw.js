/* Goallak service worker
   ...
   goalak-v44  2026-08-17  v5.0 FANTASY: the picker filter tells you what you picked, and there is a way out. (1) Choosing a league filtered the list correctly and still looked as though it had reset to All - the selected chip differed by about 4% of luminance, and paintPicker rewrites the sheet so the strip came back scrolled to its start with the leagues off-screen to the right. Selected is the accent colour now and the chosen chip is scrolled into view after every repaint. (2) Served from /fantasy/ the game is a separate page, not a tab, so the app's bottom nav is not on screen and the only exit was a browser back gesture that a home-screen PWA does not have; a back control sits in the header whenever there is an app above it. (3) The fantasy header was hardcoded Arabic markup, so the English build read جولك فانتازي in its own header.
   goalak-v43  2026-08-17  v4.9 FANTASY: the tutorial teaches instead of picking, and nothing invents a score. (1) The tutorial used to COMMIT the squad it built during the lesson - and so did Skip - so a player finished onboarding owning eleven clubs he had never chosen. It hands over an empty pitch now; picking the team is the first thing the player actually does. (2) SEASON_STARTED is false until real fixtures feed the scoring, so every total is zero, yours and every rival's, and the points screen states that the season has not started and when round one opens rather than drawing a pitch full of simulated numbers. Presenting a simulation as a scoreline was the one thing that screen must never do. (3) Round dates are words, not 25/09 - 12/10, which is a puzzle and is ambiguous to anyone who reads month-first. (4) The eight chips get their own key in the bottom nav; they were reachable only from the settings sheet, which is why the wildcards could not be found. Help gives up the slot and moves into settings - help is read once, chips are a decision every round. Fantasy remains gated to the owner.
   goalak-v42  2026-08-17  v4.8 FANTASY IS GATED AGAIN. v4.7 opened it to everyone; the owner asked for it back behind 'coming soon' until it officially starts, with his own account able to reach it. The tab shows the holding screen to every visitor and the way in appears only for the signed-in owner; /fantasy/ is a public URL so the game itself carries the same check and serves the same holding screen. This is a RELEASE GATE and not a security control - the allowlist is readable in the file and the page is still fetchable - it exists so an unfinished game is not advertised to the app's users. Real protection belongs in the worker, as chat already is. Also: the fantasy header was hardcoded Arabic markup that paintChrome never touched, so the English build still read جولك فانتازي in its own header.
   goalak-v41  2026-08-17  v4.7 FANTASY SHIPS. The Fantasy tab was a 'coming soon' star; it now opens the game at /fantasy/. Two things changed on the way in from the demo: (1) the demo wiped localStorage on EVERY page load so the owner always saw a first run - in the app that would delete a manager's squad every time they opened it, so it is removed, and squad/captain/chips/team name/ground all persist; starting over is a deliberate act in the settings sheet. (2) The disclosure copy no longer says 'demo build for review' - it says what is actually true, that POINTS ARE SIMULATED and will come from real matches once the season starts. That distinction matters: the game, the budget, the 126 clubs, the 36-round calendar and the auto-substitutions are all real and final; only the results feeding them are generated. Shipped with qa.mjs green - 252 assertions over 5 viewports x 2 languages, a suite validated by reinjecting each of the six faults this build previously shipped.
