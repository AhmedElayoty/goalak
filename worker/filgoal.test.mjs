...
const page = 'x {"TimeZoneConsidered":true,"Id":375883,"HomeTeamName":"غزل المحلة","AwayTeamName":"الشرقية إنبي","HomeScore":0,"AwayScore":0,"CurrentMatchStatusText":"over","HomeTeamCoachName":"أ","AwayTeamCoachName":"ب","HomeTeamId":7,"AwayTeamId":150,"HomeTeamFormationName":"4-3-3","HomeTeamSquad":' + JSON.stringify(Array.from({length: 11}, (_, i) => ({PersonId: 100 + i, PersonName: "لاعب " + i, ShirtNumber: i + 1, PlayerPositionName: i === 0 ? "حارس مرمى" : i < 5 ? "مدافع" : i < 8 ? "لاعب وسط" : "مهاجم", IsSpare: false, Order: i + 1, IsCaptin: i === 3}))) + ',"HomeTeamSpareSquad":[{"PersonId":200,"PersonName":"بديل","ShirtNumber":16,"PlayerPositionName":"حارس مرمى","IsSpare":true,"Order":12}],"Comments":[{"Id":1,"Time":49,"ContentUrl":"","Content":"تسديدة قوية","MatchStatusName":"الشوط الثاني"},{"Id":2,"Time":30,"ContentUrl":"<blockquote class=\\"twitter-tweet\\"><p lang=\\"ar\\">جول</p>&mdash; FilGoal (@FilGoal) <a href=\\"https://twitter.com/FilGoal/status/1234567890?ref_src=twsrc%5Etfw\\">September 2, 2026</a></blockquote>","Content":"جووول","MatchStatusName":"الشوط الأول"}],"Events":[{"MatchEventTypeName":"بطاقة صفراء","TeamName":"غزل المحلة","PlayerAName":"عمرو جمعة","Minute":12},{"MatchEventTypeName":"هدف","TeamName":"الشرقية إنبي","PlayerAName":"س","Minute":30}]} y';
const mb = parseMatchBlob(page, 375883);
eq(mb.comments.length, 2, "two commentary lines"); eq(mb.comments[0].m, 94, "a second-half comment at 49 is minute 94 (FilGoal counts from the half's start)"); eq(mb.comments[1].m, 30, "a first-half one is itself"); eq(mb.pv, 4, "parse version travels with the cache");
eq(mb.lineups.length, 2, "two line-up sides"); eq(mb.lineups[0].startXI.length, 11, "eleven starters"); eq(mb.lineups[0].startXI[0].player.pos, "G", "the keeper is G"); eq(mb.lineups[0].startXI[6].player.pos, "M", "a midfielder is M"); eq(mb.lineups[0].substitutes.length, 1, "the bench"); eq(mb.lineups[0].formation, "4-3-3", "formation travels"); eq(mb.lineups[0].startXI[3].player.captain, true, "the captain is marked"); eq(mb.comments[1].url, "https://twitter.com/FilGoal/status/1234567890", "the clip link is pulled out of the embed HTML FilGoal calls a URL (2026-09-02: the raw embed became a 404 on goallak.com)"); eq(mb.events.filter(e => e.goal).length, 1, "one goal event"); eq(mb.events[0].yellow, true, "a yellow is a yellow"); eq(mb.over, true, "finished");
